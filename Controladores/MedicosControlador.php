...
class MedicosControlador extends Conexion implements IMedicos {
    ...
    public function municipioxdepartamento($idDepartamento) {
        try {
            self::cotejamiento();
            $sql = "CALL sp_municipioxDepartamento(?);";
            $stmt = $this->cnn->prepare($sql);
    ...
    public function listarMedicamentos() {
        try {
            self::cotejamiento();
            $sql = "CALL sp_listarMedicamentos();";
            $stmt = $this->cnn->prepare($sql);
            $stmt->execute();
            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                $this->result[] = $row;
    ...
    public function listarEtapaTumor($Paciente) {
        try {
            $sql = "CALL sp_listarEtapatumores (?);";
            $stmt = $this->cnn->prepare($sql);
    ...
    public function eliminarMedicamento($idMedicamento) {
         try {
            $sql = "CALL sp_eliminarMedicamento (?);";
            $stmt = $this->cnn->prepare($sql);
            $stmt->bindParam(1, $idMedicamento);
            $stmt->execute();
    ...
    public function actualizarHorario($datos) {
        try {
            $sql = "CALL sp_actualizarHorario (?, ?, ?, ?, ?, ?, ?);";
            $stmt = $this->cnn->prepare($sql);
            $stmt->bindParam(1, $datos["idHorario"]);
            $stmt->bindParam(2, $datos["idMedico"]);
            $stmt->bindParam(3, $datos["edit_select_consultorio"]);
            $stmt->bindParam(4, $datos["edit_select_especialidad"]);
            $stmt->bindParam(5, $datos["edit_upd_fecha"]);
            $stmt->bindParam(6, $datos["edit_upd_horaInicio"]);
            $stmt->bindParam(7, $datos["edit_upd_horaFin"]);
            $stmt->execute();
...
if(isset($_GET["consultarActividadesEtapa"]) && $_GET["consultarActividadesEtapa"]=="listarActividades"){
    ...
    $r = $controller->consultarActividadesEtapa($_GET["idCita"]);
    if(count($r) != 0){
         echo '{
            "data": [';
            for($i=0;$i<count($r)-1;$i++){
...
if(isset($_GET["listarMedicamentoPaciente"]) && $_GET["listarMedicamentoPaciente"]=="listar"){
    $controller = new MedicosControlador();
    $r = $controller->listarMedicamentoPaciente($_GET["idCita"]);
    if(count($r) != 0){
         echo '{
            "data": [';
            for($i=0;$i<count($r)-1;$i++){
...
if(isset($_GET["listarDepartamentos"]) && $_GET["listarDepartamentos"]=="listar"){
    $controlador = new MedicosControlador();
    $r = $controlador->listarDepartamentos();
    echo json_encode($r);
    return;
}

if(isset($_GET["id_depamunicipio"]) && $_GET["id_depamunicipio"]!=null){
    $controlador = new MedicosControlador();
    $r = $controlador->municipioxdepartamento($_GET["id_depamunicipio"]);
    echo json_encode($r);
    return;
}

if(isset($_GET["medicamentosReceta"]) && $_GET["medicamentosReceta"]=="listar" ){
    $controlador = new MedicosControlador();
    $r = $controlador->listarMedicamentos();
    echo json_encode($r);
    return;
}
if(isset($_POST["eliminarActividad"]) && $_POST["eliminarActividad"]=="eliminar"){
    $controlador = new MedicosControlador();
    $r = $controlador->eliminarActividad($_POST["idActividad"]);
    echo json_encode($r);
    return; 
}
if(isset($_GET["listaEtapaTumor"]) && $_GET["listaEtapaTumor"]="listar"){
    $controlador = new MedicosControlador();
    $r = $controlador->listarEtapaTumor($_GET["idPaciente"]);
    echo json_encode($r);
    return; 
}
if(isset($_POST["eliminarMedicamento"]) && $_POST["eliminarMedicamento"]=="eliminar"){
    $controlador = new MedicosControlador();
    $r = $controlador->eliminarMedicamento($_POST["idMedicamento"]);
    echo json_encode($r);
    return; 
...
if(isset($_POST["editHorarioMedico"]) && $_POST["editHorarioMedico"]=="actualizarHorario"){
    //print_r($_POST);return;
    session_start();
    $datos = array(
        "idMedico" => $_SESSION["idMedico"],
        "edit_select_consultorio" => $_POST["edit_select_consultorio"],
        "edit_upd_fecha" => $_POST["edit_upd_fecha"],
        "edit_select_especialidad" => $_POST["edit_select_especialidad"],
        "edit_upd_horaInicio" => $_POST["edit_upd_horaInicio"],
        "edit_upd_horaFin" => $_POST["edit_upd_horaFin"],
